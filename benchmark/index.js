...
console.log("Portage vs Postal benchmarks");
console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
console.log("N=" + N, "\n");

var testPairs = require("fs").readdirSync("./tests").map(function(path){
    var test = require("./tests/" + path);
...
async.series(testPairs, function(err, results){
    if (err)
        return console.log("Stopped benchmarks due to an error:", err.toString());
    ...
    console.log("Portage is faster at", portageWins + "/" + results.length, "benchmarks");
    console.log("Postal is faster at", postalWins + "/" + results.length, "benchmarks");
});
